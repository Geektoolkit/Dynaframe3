...
namespace Dynaframe3.Playlist
{
  public class Engine
  {
    ...
    public List<int> Playlist { get; private set; }
    ...
    public void RebuildPlaylist(AppSettings appsettings)
    {
      ...
      appsettings.CurrentPlayList.ForEach(dir =>
      {
        using (var db = new MediaDataContext())
        {
          db.MediaFiles
            .Where(d => d.Path.StartsWith(dir))
            .ForEachAsync(file => Playlist.Add(file.Id));
        }
      });

      // Add in top level directories
      // TODO: This can be filtered out here.
      appsettings.SearchDirectories.ForEach(dir =>
      {
        using (var db = new MediaDataContext())
        {
          db.MediaFiles
            .Where(d => d.Directory == dir)
            .ForEachAsync(file => Playlist.Add(file.Id));
        }
      });
    ...
    public void AddItemsToDatabase(AppSettings appSettings, List<string> Folders, SearchOption searchOptions)
    {
      ...
      {
        exceptions = appSettings.IgnoreFolders.Split(",");
      }

      foreach (string folder in Folders) {
        var info = new DirectoryInfo(folder);

        // Skip folders that are in the exception list
        if (exceptions.Any(info.Name.StartsWith)) {
          Logger.LogComment($"FOLDER EXCEPTION: Skip folder: {folder}");
          continue;
        }

        // Skip folder that doesn't exist
        ...
        try {
          IEnumerable<string> files = Helpers
            .GetFilesByExtensions(info, searchOptions, Helpers.GetSupportedExtensions())
            // Filter 1: MAC Computers create files that end in .jpg that start with "._"
            .Where(f => !new FileInfo(f).Name.StartsWith("._"));
          ...
          using (var db = new MediaDataContext())
          {
            foreach (string file in files)
            {
              ...
              mediaFile.Type = Helpers.GetPlayListItemTypeFromPath(file).ToString();
        ...
        {
          Logger.LogComment("WARNING: Unable to load file or files from Folder " + folder + " Exception: " +
            exc.ToString()
          );
        }
      }
